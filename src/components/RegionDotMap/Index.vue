...
const geojson = computed(() => {
  return props.data.map((row) => {
    const { lat = 0, lng = 0 } = regions.value.find(region => {
      return region.area === row.area && region.region === row.region
    }) ?? {}
...
<template>
  ...
  >
    <LCircleMarker
      v-for="{ id, latLng, amount } in markers"
      :key="id"
      :lat-lng="latLng"
